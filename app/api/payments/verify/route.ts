...
export async function POST(req: Request) {
  ...
  if (!order || order.payment?.razorpayOrderId !== razorpay_order_id) return NextResponse.json({ error: "Payment mismatch" }, { status: 400 });

  const valid = await verifyRazorpayPayment(razorpay_order_id, razorpay_payment_id, razorpay_signature);
  if (!valid) return NextResponse.json({ error: "Invalid signature" }, { status: 400 });

  await prisma.$transaction([
    prisma.payment.update({ where: { orderId }, data: { razorpayPaymentId: razorpay_payment_id, status: "PAID" } }),
    prisma.order.update({ where: { id: orderId }, data: { paymentStatus: "PAID", orderStatus: "PAID" } }),
    prisma.printJob.upsert({ where: { orderId }, update: { status: "WAITING" }, create: { orderId, status: "WAITING" } })
  ]);
